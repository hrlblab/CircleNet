...
class KidPath_FirstBatch_081617_ADE(data.Dataset):
    num_classes = 1
    ...
    def __init__(self, opt, split):
        super(KidPath_FirstBatch_081617_ADE, self).__init__()
        self.data_dir = os.path.join(opt.data_dir, 'kidpath_first_batch_081617_ADE')
        self.img_dir = os.path.join(self.data_dir, '{}'.format(split))
        if split == 'test':
            self.annot_path = os.path.join(
                self.data_dir, 'kidney_first_batch_081617_ADE_test2019.json').format(split)
        else:
            if opt.task == 'exdet':
                self.annot_path = os.path.join(
                    self.data_dir, 'kidney_first_batch_081617_ADE_extreme_{}2019.json').format(split)
            else:
                self.annot_path = os.path.join(
                    self.data_dir, 'kidney_first_batch_081617_ADE_{}2019.json').format(split)
        self.max_objs = 128
        self.class_name = [
        ...
        print('==> initializing kidney_first_batch_081617_ADE 2019 {} data.'.format(split))
