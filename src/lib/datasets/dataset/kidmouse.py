...
class KidMouse(data.Dataset):
    ...
    def __init__(self, opt, split):
        super(KidMouse, self).__init__()
        self.data_dir = os.path.join(opt.data_dir, 'kidmouse')
        self.img_dir = os.path.join(self.data_dir, '{}'.format(split))
        if split == 'test':
            self.annot_path = os.path.join(
                self.data_dir, 'kidneymouse_test2019.json').format(split)
        else:
            if opt.task == 'exdet':
                self.annot_path = os.path.join(
                    self.data_dir, 'kidneymouse_extreme_{}2019.json').format(split)
            else:
                self.annot_path = os.path.join(
                    self.data_dir, 'kidneymouse_{}2019.json').format(split)
        self.max_objs = 128
        self.class_name = [
