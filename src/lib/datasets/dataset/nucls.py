...
class NuCLS(data.Dataset):
    ...
    def __init__(self, opt, split):
        super(NuCLS, self).__init__()
        self.data_dir = os.path.join(opt.data_dir, 'NuCLS')
        self.img_dir = os.path.join(self.data_dir, '{}'.format(split))
        if opt.task == 'exdet':
            self.annot_path = os.path.join(
                self.data_dir, 'NuCLS_{}2021.json').format(split)
        else:
            self.annot_path = os.path.join(
                self.data_dir, 'NuCLS_{}2021.json').format(split)
        self.max_objs = 1000
        self.class_name = [
            '__background__', 'tumor', 'fibroblast', 'lymphocyte', 'plasma_cell', 'macrophage', 'mitotic_figure',
        'vascular_endothelium', 'myoepithelium', 'neutrophil', 'apoptotic_body', 'ductal_epithelium', 'eosinophil']
    ...
    def run_circle_eval(self, results, save_dir):
        # result_json = os.path.join(save_dir, "results.json")d
        # detections  = self.convert_eval_format(results)
        # json.dump(detections, open(result_json, "w"))
