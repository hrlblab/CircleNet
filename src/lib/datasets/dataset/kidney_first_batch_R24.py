...
class KidPath_FirstBatch_R24(data.Dataset):
    ...
    def __init__(self, opt, split):
        ...
        self.data_dir = os.path.join(opt.data_dir, 'kidpath_first_batch_R24')
        self.img_dir = os.path.join(self.data_dir, '{}'.format(split))
        if split == 'test':
            self.annot_path = os.path.join(
                self.data_dir, 'kidney_%s_test2019.json'%opt.test_val_name).format(split)
        elif split == 'val':
            self.annot_path = os.path.join(
                self.data_dir, 'kidney_%s_val2019.json'%opt.test_val_name).format(split)
        else:
            if opt.task == 'exdet':
                self.annot_path = os.path.join(
                    self.data_dir, 'kidney_first_batch_R24_extreme_{}2019.json').format(split)
            else:
                self.annot_path = os.path.join(
                    self.data_dir, 'kidney_first_batch_R24_{}2019.json').format(split)
        self.max_objs = 128
        self.class_name = [
        ...
        print('==> initializing KidPath_FirstBatch_R24 2019 {} data.'.format(split))
